    ...
    def stop(self):
        ...
        try:
            ...
            if mdevice_id is not None:
                mout = self.adb.cmd("-s", mdevice_id, "shell", "ps", "-C", "uiautomator").communicate()[0].decode("utf-8").strip().splitlines()
                if len(mout) > 1:
                ...
                mout = self.adb.cmd("shell", "ps", "-C", "uiautomator").communicate()[0].decode("utf-8").strip().splitlines()
                if len(mout) > 1:
            ...
            if sdevice_id is not None:
                sout = self.adb.cmd("-s", sdevice_id, "shell", "ps", "-C", "uiautomator").communicate()[0].decode("utf-8").strip().splitlines()
                if len(sout) > 1:
                    index1 = mout[0].split().index("PID")
                    index2 = mout[0].split().index("VSIZE")
                    for line in sout[2:]:
